...
---------------------------------------------------------------------- */
#ifndef __HTU21D_EXAMPLE__
#define __HTU21D_EXAMPLE__
...
void  EXAMPLE_HTU21D(void)
{
	...
	INT32S data;
		...
		printf("-------------------- Temperature & Humidity sensor  HTU21D --------------------\r\n");
			...
			status = HTU21D_INIT();
			if(status==0) printf(" INITIAL HTU21D success!\r\n");
			else printf(" INITIAL HTU21D fail!\r\n");
			...
			if(status==0)
			{		
					if(battery_status==0x01)printf(" HTU21D Battery VDD<2.25v \r\n");
					if(battery_status==0x00)printf(" HTU21D Battery VDD>2.25v \r\n");
			}
			else 		
			{
			...
			if(status ==0)
			{
				if(mode | HTU21D_MODE0) printf("HTU21D is mode 0, 0x%x\r\n",mode);
				else printf("HTU21D is NOT mode , 0x%x\r\n",mode);
			}
			else 
			{
		...
		while(1)
			{
				...
				status = HTU21D_READ_TEMPERATURE(&temperature,mode);
				 if(status!=0) printf(" read HTU21D temperature data fail %d\r\n",status);
